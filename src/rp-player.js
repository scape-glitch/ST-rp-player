...
(function () {
  ...
  const sources = createSources({
    ...
    ytKey: ytKey,
    BAIBAI_JS: BAIBAI_JS,
    DEFAULT_YT_KEY: DEFAULT_YT_KEY,
    fetchTimeout: fetchTimeout,
    fetchWithCancel: fetchWithCancel,
    extractYtVideo: extractYtVideo,
    extractYtList: extractYtList,
    getCachedInvidious: getCachedInvidious,
    dbg: dbg,
    warn: warn
  });
  const Jamendo = sources.Jamendo;
  const YouTubeAPI = sources.YouTubeAPI;
  const YouTubeLink = sources.YouTubeLink;
  const Baibai = sources.Baibai;
  const CCMixter = sources.CCMixter;
  const FMA = sources.FMA;
  const ALL_SOURCES = sources.ALL_SOURCES;
  const srcByKey = sources.srcByKey;
