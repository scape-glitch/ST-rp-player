(function () {
  ...
  function fabHTML() {
    const cur = curTrack();
    const playingSomething = (curIdx >= 0) || !!window.__rpRadioNow;
    const showVibe = rpEnabled() && cfg.rpQuick && !playingSomething;
    let inner;
    if (showVibe) {
  ...
  function rpHTML() {
    ...
    if (rpAccSource) {
      h += '<div class="' + PFX + '-acc-body"><div class="' + PFX + '-srcrow">' +
        '<div class="' + PFX + '-srcbtn ' + (cfg.rpSource === 'auto' ? 'on' : '') + '" data-rpsrc="auto">Авто (все)</div>' +
  ...
  function updatePlayBtn() {
    ...
    if (uiMode === 'fab') {
      ...
      const playingSomething = (curIdx >= 0) || !!window.__rpRadioNow;
      const wantVibe = rpEnabled() && cfg.rpQuick && !playingSomething;
      const haveVibe = !!(fb && fb.getAttribute('data-fabvibe'));
      if (wantVibe !== haveVibe) render();
      return;
    }
    if (collapsed) {
      const cur = curTrack();
  ...
  function onClick(e) {
    ...
    if (t.closest('[data-editfav]')) { editKind = 'fav'; editId = '__fav__'; scrollMemory.lib = 0; render(); lockUntil = Date.now() + 300; return; }
    ...
    if ((m = t.closest('[data-editm]'))) { editId = m.getAttribute('data-editm'); editKind = 'm'; exportPop = null; scrollMemory.lib = 0; render(); lockUntil = Date.now() + 300; return; }
    if ((m = t.closest('[data-editrp]'))) { editId = m.getAttribute('data-editrp'); editKind = 'rp'; exportPop = null; scrollMemory.lib = 0; render(); lockUntil = Date.now() + 300; return; }
